...
// 테스트레이블, 체력등 이미지, 페이지 넘버...
   ...
   override func viewDidLoad() {
    ...
    //캐릭터 현재 페이지 위치를 choice nextPageIndex 이용해서 업데이트함. 1 뺀 이유는 실제 인덱스값이랑 currentPageNumber가 1 차이 나서.
        santaCharacter1.currentPageNumber = currentPage.choice[indexPath.row].nextPageIndex-1
    // 페이지 storytext를 위 값 이용해 업뎃
        testLable.text = "\(prologueEP[santaCharacter1.currentPageNumber].storyText)"
    // 선택지 테이블 뷰 새로고침(이나.. 현재까지 작동 안 되는 중)
        self.choiceTableView.beginUpdates()
        self.choiceTableView.reloadData()
        self.choiceTableView.endUpdates()
    // 캐릭터
